...
export function formatCashBookBillInfo(billNo?: string, billDate?: string): string {
  ...
  const cleanBillNo = bNo.replace(/^bill\s*\/?\s*invoice\s*#?\s*:?\s*/i, '').trim();
  if (!cleanBillNo) return '';

  const hasValidDate = bDate && bDate !== '—' && bDate !== '-' && bDate.toUpperCase() !== 'N/A';
  if (hasValidDate) {
    return `Bill/Invoice#: ${cleanBillNo} (${bDate})`;
  }
  return `Bill/Invoice#: ${cleanBillNo}`;
}
...
export function generateOfficialStatementPrintHtml(params: {
  ...
  const sourceLabel =
    params.sourceLabel ||
    (params.reportType === 'CASHBOOK'
      ? 'Source: Reports & Statements → Cashbook Statement'
      : params.reportType === 'HEAD'
      ? 'Source: Reports & Statements → Head-wise Expenditure Statement'
      : `Source: Reports & Statements → ${params.title}`);
      ...
      rowsHtml += `
        <tr style="border-bottom: 1px solid #e2e8f0;">
          ...
          <td style="padding: 4px; border: 1px solid #cbd5e1; font-size: 8.5px; word-break: break-word;">
            ...
            ${(params.reportType === 'CASHBOOK' || params.reportType === 'HEAD') && formatCashBookBillInfo(r.billNo, r.billDate) ? `<div style="font-family: monospace; font-size: 7.5px; color: #0f172a; font-weight: normal; line-height: 1.2; margin-top: 2px;">${formatCashBookBillInfo(r.billNo, r.billDate)}</div>` : ''}
          </td>
      ...
      <body>
        ...
        <div class="report-header-wrap">
          ...
          <div class="header-text-block">
            <div style="font-size: 7.5px; font-family: monospace; color: #475569; font-weight: 700; margin-bottom: 2px;">${sourceLabel}</div>
        ...
        <div class="official-footer-strip">
          <div>e-CashBook &amp; Voucher System developed by MKZ for institute 33028</div>
...
export function generateMultiHeadStatementPrintHtml(params: {
  ...
  const sourceLabel = params.sourceLabel || 'Source: Reports & Statements → Head-wise Expenditure Statement';
        ...
        perHeadRowsHtml += `
          <tr>
            ...
            <td style="padding: 4px; border: 1px solid #cbd5e1; font-size: 8px;">
              ...
              ${formatCashBookBillInfo(r.billNo, r.billDate) ? `<div style="font-family: monospace; font-size: 7px; color: #0284c7; margin-top: 1px;">${formatCashBookBillInfo(r.billNo, r.billDate)}</div>` : ''}
            </td>
